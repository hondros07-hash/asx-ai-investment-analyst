...
export default function UpcomingDividendsGrid({marketRegion="AU",payload,isLoading=false,onRetry}){
 const rows=payload?.dividend_calendar||[]; const status=payload?.status||"UNAVAILABLE"; const au=marketRegion.toUpperCase()==="AU";
 const sources=useMemo(()=>[...new Set(rows.map(x=>x.source).filter(Boolean))],[rows]);
 if(isLoading)return <Skeleton/>;
 const unavailable=["UNAVAILABLE","PROVIDERS_NOT_CONFIGURED","INVALID_MARKET"].includes(status);
 return <section className="overflow-hidden rounded-xl border border-[#d8e5f2] bg-white shadow-[0_1px_3px_rgba(8,43,85,.05)]">
  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e6edf5] px-5 py-4">
   <div><h2 className="text-[15px] font-extrabold tracking-tight text-[#082b55]">Upcoming Dividends — {payload?.market_name||marketRegion}</h2><p className="mt-1 text-[11px] text-slate-500">Confirmed declared corporate actions · Chrímata does not estimate undeclared dividends</p></div>
  ...
  {rows.length===0?<div className="px-6 py-12 text-center"><div className="mx-auto max-w-xl text-sm font-semibold text-[#274a72]">{unavailable?"Dividend data unavailable":"No confirmed upcoming dividends found"}</div><p className="mx-auto mt-2 max-w-2xl text-xs leading-5 text-slate-500">{unavailable?"Chrímata could not establish a verified forward calendar from the configured provider chain. This is not a claim that no companies are paying dividends.":"The configured providers returned no verified declared events in the current horizon for this market."}</p>{onRetry&&<button onClick={onRetry} className="mt-4 rounded-lg border border-[#cbdced] px-3 py-1.5 text-xs font-bold text-[#0b63ce] hover:bg-[#f5f9fd]">Retry</button>}</div>:
  ...
  <div className="flex flex-wrap justify-between gap-2 border-t border-[#e6edf5] bg-[#fbfdff] px-5 py-2.5 text-[10px] text-slate-500"><span>{rows.length} confirmed event{rows.length===1?"":"s"} · 15-minute cache</span><span>{sources.length?`Sources: ${sources.join(", ")}`:"Provider provenance retained by API"} · AI calculated math: False</span></div>
 </section>
}
