...
def test_native_account_bar_exists():
 s=Path("app.py").read_text(encoding="utf-8")
 assert ('with st.container(key="v230032_account_bar")' in s or 'with st.container(key="v2322_market_strip")' in s)
...
def test_account_bar_is_compact_and_right_aligned():
 ...
 assert ('st.columns([12,1.05,1.15]' in s or 'st.columns([10.8,1.05,1.15]' in s or '_m1,_m2,_m3,_m4,_m5,_signin,_register=st.columns' in s)
