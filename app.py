...
- Obtain legal advice on the exact Australian licensing/authorisation model before offering execution to other users.""")
    ...
    if h.empty:
        ...
        _strip_html=''.join([
            _research_card,
            f'<div class="v21262-strip-card blue v21269-valuation-card"><div class="v21262-strip-icon v21269-val-icon">▣</div><div class="v21262-strip-copy v21269-val-copy"><div class="v21262-strip-title">Valuation</div><div class="v21269-val-value">{html.escape(_val_label)}</div><div class="v21269-val-base">{html.escape(f"Base case: {display_price(_ccbase,ticker)}" if np.isfinite(_mia_num(_ccbase)) else "Base case unavailable")}</div><div class="v21269-val-move {_val_move_cls}">{html.escape(_val_move)}</div><div class="v21269-val-confidence">{html.escape(_val_conf_text)}</div></div></div>',
        ...
        st.markdown('<div class="v21-foot">V21.2.69 · Valuation Widget + Validation Confidence. Values are drawn from the selected company’s loaded provider, model and stored evidence; unsupported fields remain unavailable rather than being fabricated.</div>',unsafe_allow_html=True)
