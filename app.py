...
- Obtain legal advice on the exact Australian licensing/authorisation model before offering execution to other users.""")
    ...
    if h.empty:
        ...
        st.markdown("""<style>
        .v2121-shell{background:#fff;border:1px solid #d9e5f2;border-radius:10px 10px 0 0;box-shadow:0 1px 3px rgba(16,38,75,.04);margin:0;overflow:hidden;border-bottom:0}.v2121-top{display:grid;grid-template-columns:minmax(330px,1.35fr) minmax(190px,.72fr) minmax(440px,1.35fr);align-items:center;gap:10px;padding:8px 14px;border-bottom:1px solid #e4edf7;min-height:54px}.v2121-cc-title{font-size:19px;font-weight:900;color:#10264b;line-height:1.05}.v2121-cc-sub{font-size:11px;color:#10264b;margin-top:4px}.v2121-sep{color:#c4d1df;margin:0 9px}.v2121-motto{text-align:center;font-size:10px;color:#647b97}.v2121-company{display:grid;grid-template-columns:105px minmax(220px,1.25fr) minmax(105px,.72fr) minmax(150px,.95fr) minmax(110px,.72fr) minmax(105px,.68fr) minmax(95px,.62fr) minmax(110px,.70fr) minmax(85px,.56fr) minmax(150px,.9fr) 88px;align-items:center;padding:9px 14px;background:#fff;border:1px solid #d9e5f2;border-top:0;border-radius:0 0 10px 10px;box-shadow:0 1px 3px rgba(16,38,75,.04);margin-bottom:7px}.v2121-logo{display:flex;align-items:center;padding-right:15px}.v2121-logo img{max-width:92px;max-height:52px;object-fit:contain}.v2121-fallback{width:88px;height:52px;border-radius:10px;background:#eef5ff;color:#1458aa;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900}.v2121-ident{padding-right:16px}.v2121-name{font-size:17px;font-weight:900;color:#10264b}.v2121-ticker{display:inline-block;background:#edf4ff;color:#10264b;border-radius:6px;padding:4px 8px;margin-left:7px;font-size:11px;font-weight:900}.v2121-meta,.v2121-desc{font-size:10px;color:#55708e;margin-top:5px}.v2121-desc{font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:310px}.v2121-stat{min-height:56px;border-left:1px solid #e1e9f2;padding:3px 10px;display:flex;flex-direction:column;justify-content:center}.v2121-k{font-size:9px;font-weight:800;color:#637b98}.v2121-v{font-size:14px;font-weight:900;color:#10264b;margin-top:3px;white-space:nowrap}.v2121-up{color:#069a4e;font-size:10px;font-weight:900}.v2121-down{color:#d14343;font-size:10px;font-weight:900}.v2121-range{height:4px;border-radius:4px;background:#dce6f1;position:relative;margin-top:7px}.v2121-range-dot{position:absolute;top:-3px;width:10px;height:10px;border-radius:50%;background:#0878ed;transform:translateX(-50%)}.v2121-update .v2121-v{font-size:10px;font-weight:700;line-height:1.35}.v2121-live{font-size:9px;font-weight:800;color:#078d4c;white-space:nowrap}.v2121-dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:#08a45c;margin-right:5px}
        ...
        _range_pos=50.0 if _cchi<=_cclo else max(0.0,min(100.0,(price-_cclo)/(_cchi-_cclo)*100.0)); _logo_candidates=company_logo_candidates(ticker,_ccmeta,192); _initials="".join([x[0] for x in str(_ccname).split()[:2] if x])[:2].upper() or str(ticker)[:2].upper()
        if _logo_candidates:
            _src=html.escape(_logo_candidates[0],quote=True); _logo_html=f'<img src="{_src}"><div class="v2121-fallback" style="display:none">{html.escape(_initials)}</div>'
        else: _logo_html=f'<div class="v2121-fallback">{html.escape(_initials)}</div>'
        _desc=str(_ccmeta.get("longBusinessSummary") or ""); _desc=(_desc[:102].rstrip()+"…") if len(_desc)>105 else _desc; _currency=str(_ccmeta.get("currency") or "")
        _mcap_txt=compact_number(_ccmcap,prefix=("A$" if _currency=="AUD" else "$")) if np.isfinite(_ccmcap) else "—"; _vol_txt=compact_number(_ccvol) if np.isfinite(_ccvol) else "—"; _pe_txt=f"{_ccpe:.1f}×" if np.isfinite(_ccpe) else "—"; _dy_txt=f"{_ccdy:.2%}" if np.isfinite(_ccdy) else "—"; _beta_txt=f"{_ccbeta:.2f}" if np.isfinite(_ccbeta) else "—"; _updated=pd.Timestamp.now(tz="Australia/Melbourne").strftime("%d %b %Y %I:%M%p AEST").replace(" 0"," ")
        st.markdown(f"""<div class="v2121-shell"><div class="v2121-top"><div><div class="v2121-cc-title">⚯&nbsp; Company Command Centre</div><div class="v2121-cc-sub"><b>Overview</b><span class="v2121-sep">|</span>{html.escape(str(ticker))} – {html.escape(str(_ccname))}</div></div><div class="v2121-motto">All the evidence. A clearer perspective.</div><div></div></div></div>""",unsafe_allow_html=True)
        st.markdown("""<style>
        div[data-testid="stHorizontalBlock"]:has(button[kind="secondary"]) {gap:.55rem}
        .v2121-actions{height:0;position:relative;z-index:8}
        </style><div class="v2121-actions"></div>""",unsafe_allow_html=True)
        _hb1,_hb2,_hb3,_hb4=st.columns([5.7,1.15,1,1.45])
        with _hb2:
            if st.button("☆  Add to Watchlist",key=f"v2121_watch_{ticker}",use_container_width=True): watch_add(ticker,""); st.toast(f"{ticker} added to Watchlist")
        with _hb3: st.button("↗  Compare",key=f"v2121_compare_{ticker}",use_container_width=True,help="Comparison workspace integration follows in the next research-engine build.")
        with _hb4: st.button("Open in new tab  →",key=f"v2121_newtab_{ticker}",use_container_width=True,help="Use your browser tab controls to open Chrímata in another tab.")
        st.markdown("""<style>
        div[data-testid="stHorizontalBlock"]:has(button[key*="v2121_"]) {margin-top:-51px!important;margin-bottom:5px!important;padding-right:13px!important;position:relative;z-index:9}
        div[data-testid="stHorizontalBlock"]:has(button[key*="v2121_"]) button{min-height:38px!important;height:38px!important;border-radius:7px!important;font-size:12px!important;font-weight:800!important;padding:.25rem .65rem!important}
        </style>""",unsafe_allow_html=True)
        st.markdown(f"""<div class="v2121-company"><div class="v2121-logo">{_logo_html}</div><div class="v2121-ident"><div class="v2121-name">{html.escape(str(_ccname))}<span class="v2121-ticker">{html.escape(str(ticker))}</span></div><div class="v2121-meta">{html.escape(str(_ccsector))}<span class="v2121-sep">|</span>{html.escape(str(_ccindustry))}</div><div class="v2121-desc">{html.escape(_desc)}</div></div><div class="v2121-stat"><div class="v2121-k">Share Price</div><div class="v2121-v">{display_price(price,ticker)}</div><div class="{_delta_cls}">{html.escape(_deltatxt)}</div></div><div class="v2121-stat"><div class="v2121-k">52 Week Range</div><div class="v2121-v">{display_price(_cclo,ticker)} – {display_price(_cchi,ticker)}</div><div class="v2121-range"><span class="v2121-range-dot" style="left:{_range_pos:.1f}%"></span></div></div><div class="v2121-stat"><div class="v2121-k">Market Cap</div><div class="v2121-v">{html.escape(_mcap_txt)}</div></div><div class="v2121-stat"><div class="v2121-k">Volume (Avg)</div><div class="v2121-v">{html.escape(_vol_txt)}</div></div><div class="v2121-stat"><div class="v2121-k">P/E (TTM)</div><div class="v2121-v">{html.escape(_pe_txt)}</div></div><div class="v2121-stat"><div class="v2121-k">Dividend Yield</div><div class="v2121-v">{html.escape(_dy_txt)}</div></div><div class="v2121-stat"><div class="v2121-k">Beta (5Y)</div><div class="v2121-v">{html.escape(_beta_txt)}</div></div><div class="v2121-stat v2121-update"><div class="v2121-k">Last Updated</div><div class="v2121-v">{html.escape(_updated)}</div></div><div class="v2121-live"><span class="v2121-dot"></span>Live data</div></div>""",unsafe_allow_html=True)
