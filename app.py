...
st.sidebar.markdown("""<div class="chr-side-spacer"></div><div class="chr-side-wealth"><span class="wealth-pillar"><svg viewBox="0 0 48 64" aria-hidden="true"><path d="M8 8h32M11 12h26M14 16h20M14 48h20M11 52h26M8 56h32"/><path d="M16 17v30M22 17v30M26 17v30M32 17v30"/><path d="M10 6h28l-3-3H13zM10 58h28l3 3H7z"/></svg></span><span class="wealth-copy">KNOWLEDGE<br>COMPOUNDS<br>WEALTH</span></div><div class="chr-side-version">v20.3.7</div><div class="chr-side-copyright">© 2026 Chrímata. All rights reserved.</div>""",unsafe_allow_html=True)
...
def _chr_smooth_market_component(market, instruments, selected_key, range_map, sectors, index_table, date_label, time_label, zone_label, market_status, market_is_open, updated_label, panel_asof):
    # V20.3.7 stable client-side market cards and chart; fixed geometry and no timed Streamlit reruns.
...
/* V20.3.7 — stable client-side market section: no timed Streamlit reruns or layout reflow. */
...
def render_global_market_overview():
    ...
    maxabs=max([abs(v) for _,v in sectors] or [1]); sector_html=''.join(f'<div class="chr-sector-row"><span>{n}</span><i><b class="{"up" if v>=0 else "down"}" style="width:{max(3,abs(v)/maxabs*100):.0f}%"></b></i><em class="{"pos" if v>=0 else "neg"}">{v:+.2f}%</em></div>' for n,v in sectors[:11])
    index_rows=[]
    ...
    # V20.3.7: no timed Streamlit fragment rerun. The market clock updates client-side so the page remains stationary.
...
# V20.3.7 stable labelled chart axis alignment.
st.markdown(r"""<style>
