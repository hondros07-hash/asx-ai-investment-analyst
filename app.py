...
- Obtain legal advice on the exact Australian licensing/authorisation model before offering execution to other users.""")
    ...
    if h.empty:
        ...
        # Yahoo's search endpoint often retains identity/classification even when .info is thin.
        if not _ccname or str(_ccname).strip().upper() in _invalid_names or not (_ccmeta.get("sector") and _ccmeta.get("industry")):
            try:
                ...
                if _exact:
                    ...
                    for _k in ("sector","industry","sectorDisp","industryDisp","exchange","exchDisp","quoteType"):
                        if _exact.get(_k) not in (None,"","—") and _ccmeta.get(_k) in (None,"","—"): _ccmeta[_k]=_exact.get(_k)
            except Exception: pass
