...
- Obtain legal advice on the exact Australian licensing/authorisation model before offering execution to other users.""")
    ...
    if h.empty:
        ...
        _strip_html=''.join([
            ...
            f'<div class="v21262-strip-card good v21272-analyst-card"><div class="v21262-strip-icon v21272-analyst-icon" aria-hidden="true"><svg viewBox="0 0 24 24" role="img"><path d="M8.2 10.1a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm7.6 0a2.55 2.55 0 1 0 0-5.1 2.55 2.55 0 0 0 0 5.1ZM3.5 19.2v-2.1c0-2.8 2.1-5 4.7-5s4.7 2.2 4.7 5v2.1H3.5Zm9.8 0v-1.7c0-1.8-.6-3.4-1.6-4.6a4.8 4.8 0 0 1 4.1-2.2c2.6 0 4.7 2.1 4.7 4.7v3.8h-7.2Z" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div class="v21262-strip-copy v21272-analyst-copy"><div class="v21262-strip-title">Analyst Consensus</div><div class="v21272-analyst-value">{html.escape(_an_label)}</div><div class="v21272-analyst-count">{html.escape(f"{_an_n} analysts" if _an_n else "Analyst count unavailable")}</div><div class="v21272-analyst-target">{(f"Target: {html.escape(display_price(_an_target,ticker))} <span class=\"{'up' if _an_up >= 0 else 'down'}\">({html.escape(f'{_an_up:+.0%}')})</span>" if np.isfinite(_an_target) and np.isfinite(_an_up) else "Target unavailable")}</div></div></div>',
