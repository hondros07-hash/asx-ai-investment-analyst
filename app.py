...
- Obtain legal advice on the exact Australian licensing/authorisation model before offering execution to other users.""")
    ...
    if h.empty:
        ...
        if not _ccname or str(_ccname).strip().upper() in {str(ticker).upper(), str(ticker).split(".")[0].upper()}:
            try:
                _rn=company_name(ticker) if 'company_name' in globals() else None
                if _rn and str(_rn).strip().upper() not in {str(ticker).upper(), str(ticker).split(".")[0].upper()}: _ccname=_rn
            except Exception: pass
        ...
        _range_pos=50.0 if _cchi<=_cclo else max(0.0,min(100.0,(price-_cclo)/(_cchi-_cclo)*100.0)); _logo_candidates=company_logo_candidates(ticker,_ccmeta,192); _initials="".join([x[0] for x in str(_ccname).split()[:3] if x])[:3].upper() or str(ticker).split(".")[0][:3].upper()
        if _logo_candidates:
        ...
        st.markdown(f"""<div class="v2121-shell"><div class="v2121-top"><div><div class="v2121-cc-title">⚯&nbsp; Company Command Centre</div><div class="v2121-cc-sub"><b>Overview</b><span class="v2121-sep">|</span>{html.escape(str(ticker))} – {html.escape(str(_ccname))}</div></div><div class="v2121-motto">All the evidence. A clearer perspective.</div><div class="v2123-actions"><a class="v2123-btn" href="#watchlist">☆&nbsp; Add to Watchlist</a><a class="v2123-btn" href="#compare">↗&nbsp; Compare</a><a class="v2123-btn primary" href="{_cc_href}" target="_blank">Open in new tab&nbsp; →</a></div></div></div>""",unsafe_allow_html=True)
