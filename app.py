...
def valuation_statement_bundle(ticker):
    ...
    try:
        t=yf.Ticker(ticker)
        # Keep quarterly and annual frames separate. A broad quarterly statement can
        # omit FCF/capex rows available in annual data; choosing by row count loses them.
        def combine(quarterly, annual):
            q=quarterly if isinstance(quarterly,pd.DataFrame) else pd.DataFrame()
            y=annual if isinstance(annual,pd.DataFrame) else pd.DataFrame()
            if q.empty:return y.copy()
            if y.empty:return q.copy()
            # Preserve both sets of dated columns and prefer the most recent observation
            # per row. No addition of quarterly and annual periods is performed.
            combined=pd.concat([q,y],axis=1)
            return combined.loc[:,~combined.columns.duplicated()]
        return (combine(getattr(t,"quarterly_cashflow",pd.DataFrame()),getattr(t,"cashflow",pd.DataFrame())),
                combine(getattr(t,"quarterly_balance_sheet",pd.DataFrame()),getattr(t,"balance_sheet",pd.DataFrame())),
                combine(getattr(t,"quarterly_financials",pd.DataFrame()),getattr(t,"financials",pd.DataFrame())))
    except Exception:
